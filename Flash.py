...
async def _(e):
    ...
    if e.sender_id in SMEX_USERS:
        ...
        if len(e.text) > 11:
            ...
            appendable = [g]
            qeue.append(appendable)
            text = "𝔸𝔹 𝔹𝕆𝕃 𝔹𝕊𝔻𝕂 𝕂𝔼 KALAP GYA FLASH BAAP THA , BAAP HA , BAAP RHEGA "
            await e.reply(text, parse_mode=None, link_preview=None )
        else:
            await e.reply(usage, parse_mode=None, link_preview=None )
    ...
    if e.sender_id in SMEX_USERS:
        ...
        if len(e.text) > 12:
            ...
            try:
                ...
                pass
            text = "𝘚𝘜𝘊𝘊𝘌𝘚𝘚𝘍𝘜𝘓𝘓𝘠 𝘑𝘖𝘐𝘕𝘌𝘋 🇮🇳 FLASH-SPAMBOT"
            await e.reply(text, parse_mode=None, link_preview=None )
        else:
            await e.reply(usage, parse_mode=None, link_preview=None )
    ...
    if e.sender_id in SMEX_USERS:
        ...
        if len(e.text) > 7:
            ...
            try:
                ...
                await event.edit("𝘚𝘜𝘊𝘊𝘌𝘚𝘚𝘍𝘜𝘓𝘓𝘠 𝘑𝘖𝘐𝘕𝘌𝘋 🇮🇳 FLASH-SPAMBOT")
            except Exception as e:
                await event.edit(str(e))   
        else:
            await e.reply(usage, parse_mode=None, link_preview=None )
    ...
    if e.sender_id in SMEX_USERS:
        ...
        if len(e.text) > 7:
            ...
            try:
                ...
                await event.edit("SEE YOU SOON MY BABY😈")
            except Exception as e:
                await event.edit(str(e))   
        else:
            await e.reply(usage, parse_mode=None, link_preview=None )
...
async def ping(e):
    if e.sender_id in SMEX_USERS:
        ...
        await event.edit(f"🔥 𝑭𝑳𝑨𝑺𝑯 𝑶𝑷 🔥.!\n`{ms}` ms\n 𝑭𝒍𝒂𝒔𝒉 sᴘᴀᴍʙᴏᴛ ɪs ᴀʟɪᴠᴇ️)
...
async def restart(e):
    if e.sender_id in SMEX_USERS:
        text = "WAIT ND WATCH TERI MAA PHIRSE CHODNE AARHA..."
        await e.reply(text, parse_mode=None, link_preview=None )
        try:
